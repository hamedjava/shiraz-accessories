...
export class SellerRepository {
  ...
  /* ---------------------------------------------------
     📋 تمام فروشندگان برای پنل مدیر (اصلی)
  --------------------------------------------------- */
  async findAll() {
    try {
      return await this.model
  ...
  /* ---------------------------------------------------
     🧩 سازگاری با ساختار قبلی UseCase‌ها
     این دو متد فقط ارجاعی هستند تا خطا رفع شود
  --------------------------------------------------- */
  async find(filter = {}) {
    return await this.model.find(filter).select("name email mobile storeName role isVerified createdAt updatedAt");
  }
